...
-define (EXTENSIONS, "es_").
-define (MIME_TYPES, "ms_").
...
from_extension("." ++ ExtensionWithoutPeriod) ->
  from_extension(ExtensionWithoutPeriod);
from_extension(Extension) ->
  jiffy_global:get_string_key(?EXTENSIONS ++ Extension).

-spec from_mime_type(mime_type()) -> maybe_something(extension()).
from_mime_type(MimeType) ->
    jiffy_global:get_string_key(?MIME_TYPES ++ MimeType).

populate_ets(EtsTableName) ->
  PrefixTableName = prefix(EtsTableName),
  lists:foreach(
    fun({Key, Value}) ->
       JiffyGlobalKey = PrefixTableName ++ Key,
       jiffy_global:put_string_key(JiffyGlobalKey, Value)
    end,
    minmay:EtsTableName()),
  ok.
